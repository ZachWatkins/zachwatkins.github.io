...
export function createMenu(args) {
  var menu = {
    file: {
      save: function () {
	return $('<li class="remarklet-save-export">Save</li>');
      },
      export: function () {
	return $('<li class="remarklet-menu-export">Menu</li>');
      },
      open: function () {
	return $('<li class="remarklet-open-export">Open</li>');
      },
    },
    view: {
      grid: function () {
	return $('<li class="remarklet-open-grid">Grid</li>');
      },
      outlines: function () {
	return $('<li class="remarklet-open-outlines">Outlines</li>');
      },
      css: function () {
	return $('<li class="remarklet-open-css">CSS</li>');
      },
    },
    add: {
      image: function () {
            ...
            if (selection.length === 0 && lastchar.match(/;|{/)) {
              ...
              if (lastchar == '{' && nextchar == '}') {
                ...
                $t.insertText(
                  addtext,
                  selection.start +
                    _stored.preferences.CSS_Editor.Indentation.length +
                    1,
                  'collapseToStart',
                );
              }
    ...
    File: {
      Export: function () {
        ...
        html = html
          ...
          .replace(/(src|href)=("|')\/\//g, '$1=$2' + location.protocol)
          .replace(
            /(src|href)=("|')(\/|(?=[^:]{6}))/gi,
            '$1=$2' +
              location.protocol +
          ...
          .replace(/\s?remarklet-show-(grid|outlines|usercss)\s?|\s/g, ' ');
    ...
    Insert: {
      ...
      Note: function () {
        ...
        prompt.open({
          ...
          init: function () {
            console.log('prompt note init');
            $b.off('mousemove', _mouse.update);
          },
          callback: function (data) {
            console.log('prompt note callback');
            _stored.editcounter++;
