export function createSvg() {
  ...
  svg.mouseMove = function (e) {
    ...
    if (this.mouse.mode.name == 'drag') {
      ...
      switch (this.mouse.target.tagName) {
        case 'line':
          this.mouse.target.setAttribute(
            'x1',
            this.drag.target.p1.x + change.x,
          );
          this.mouse.target.setAttribute(
            'y1',
            this.drag.target.p1.y + change.y,
          );
          this.mouse.target.setAttribute(
            'x2',
            this.drag.target.p2.x + change.x,
          );
          this.mouse.target.setAttribute(
            'y2',
            this.drag.target.p2.y + change.y,
          );
          break;
        case 'circle':
  ...
  // Initialize.
  svg.el.setAttribute('xmlns', svg.ns);
  svg.el.setAttribute('xmlns:xlink', 'http://www.w3.org/1999/xlink');

  var title = document.createElementNS(svg.ns, 'title');
  title.textContent = 'Generative Art from zacharywatkins.com';
  svg.el.appendChild(title);

  var desc = document.createElementNS(svg.ns, 'desc');
  desc.textContent =
    'By Zachary Watkins, Math.random(), and maybe you if you changed the input parameters.';
  svg.el.appendChild(desc);

  var grid = document.getElementById('grid');
  svg.setdimensions(grid.offsetWidth);
  svg.el.style.cssText =
    'position:absolute;z-index:999;background-color:rgba(255,255,255,0.9);';
  grid.parentNode.insertBefore(svg.el, grid);
