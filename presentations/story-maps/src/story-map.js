...
StoryMap.prototype = {
  ...
  setStoryProps: function () {
    ...
    const width = Math.max(
      minWidth,
      Math.min(maxWidth, Math.round(window.innerWidth / 3)),
    );
    this.story = {
      width: width,
  ...
  wrapGoal: function () {
    ...
    this.goal.appendChild(
      this.root.childNodes[0].childNodes[0].cloneNode(true),
    );
    this.root.childNodes[0].replaceChild(
      this.goal,
      this.root.childNodes[0].childNodes[0],
    );
  },
  /**
  ...
  makeDraggable: function () {
    ...
    function isDraggable(element) {
      return (
        ...
        !element.classList.contains('goal')
      );
    }
  ...
  observeEditable: function () {
    ...
    this.root.addEventListener('keyup', (event) => {
      ...
      if (
        this.editing &&
        event.code === 'Escape' &&
        event.target.classList.contains('story')
      ) {
        this.editing = false;
        const stories = this.root.querySelectorAll('.story');
        for (let i = 0; i < stories.length; i++) {
  ...
  observeAddRemoveButtons: function () {
    ...
    buttonsWrapper.style.cssText =
      'position: relative; height: 1px; margin-bottom: -1px; text-align: right; padding-right: 3px;';

    const buttonStyles =
      'display: inline-block; padding: 0; margin-left: 2px; font: bold 18pt/1px monospace; width: 20px; height: 20px; vertical-align: middle; text-align: center; box-sizing: border-box; position: relative; top: -27px; background-color: #000; color: #FFF;';
    ...
    addButton.addEventListener('click', (event) => {
      if (event.target.parentNode.previousSibling.classList.contains('step')) {
        ...
        event.target.parentNode.previousSibling.classList.contains('task')
      ) {
