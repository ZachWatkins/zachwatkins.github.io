      ...
      if (value != cloneStyle.getPropertyValue(prop)) {
        switch (prop) {
          ...
          case '-webkit-column-rule-color':
            if (
              elStyle.getPropertyValue(prop.replace('color', 'width')) == '0px'
            ) {
              ignore = true;
            }
            break;
          ...
          case '-webkit-column-gap':
            if (
              elStyle.getPropertyValue(prop.replace('gap', 'count')) ==
                'auto' &&
              elStyle.getPropertyValue(prop.replace('gap', 'width')) == 'auto'
            ) {
              ignore = true;
            }
            break;
          ...
          case 'perspective-origin':
            if (
              elStyle.getPropertyValue(prop.replace('-origin', '')) == 'none'
            ) {
              ignore = true;
            }
            break;
      ...
      for (i = 0; i <= len; i++) {
        ...
        if (classname && typeof selectorSettings.classname === 'string') {
          cloneSelector = cloneSelector.replace(
            classname[0],
            classname[0].match(new RegExp(selectorSettings.classname, 'i')),
          );
        }
